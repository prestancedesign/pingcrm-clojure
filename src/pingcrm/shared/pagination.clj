...
(defn pagination-links [uri current-page total per-page]
  (let [uri (str uri "?page=")
        ...
        previous-link {:url (when (> current-page 1) (str uri (dec current-page))) :label "&laquo; Previous" :active (when (> current-page 1) true)}
        next-link {:url (when (< current-page page-number) (str uri (inc current-page))) :label "Next &raquo;" :active (when (<= current-page (dec page-number)) true)}
